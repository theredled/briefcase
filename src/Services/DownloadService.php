...
class DownloadService implements EventSubscriberInterface
{
    ...
    public function buildZipFromFolder(Document $document)
    {
        ...
        if ($filesAreAhead) {
            ...
            if ($ret = true !== $zip->open($zipPath, \ZipArchive::CREATE)) {
                throw new \Exception('Erreur Zip : '.$ret.', '.$zip->getStatusString());
            }
            $zip->addEmptyDir($dirname);
            if (!count($filesInFolder) and 0 == $document->getIncludedFiles()->count()) {
     ...
     */
    public function findEntityOrFail($token, ?string $lang, Request $request): ?Document
    {
        ...
        if ($document->getSensible() and !$this->uriSignerCheckRequest($request)) {
            throw new LinkExpiredException('Lien non valide ou expiré : ' . $token);
